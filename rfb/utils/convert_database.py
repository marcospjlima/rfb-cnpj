...
class ConvertDatabase:
    ...
    def drop_tables(self):
        """
        Exclui as tabelas da bases de dados
        """
        Empresa().metadata.drop_all(self.engine)
        Estabelecimento().metadata.drop_all(self.engine)
        DadoSimples().metadata.drop_all(self.engine)
        Socio().metadata.drop_all(self.engine)
        Cnae().metadata.drop_all(self.engine)
        Pais().metadata.drop_all(self.engine)
        Municipio().metadata.drop_all(self.engine)
        Qualificacao().metadata.drop_all(self.engine)
        Natureza().metadata.drop_all(self.engine)
        MotivoCadastral().metadata.drop_all(self.engine)
